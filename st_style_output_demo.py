...
uploaded_file = st.file_uploader(
    "Upload PDF report here:", type=["pdf"], key="single_pdf"
)
        ...
        while attempts < max_attempts and not achieved:
            try:
                ...
                network = create_keyword_network(
                    sized_down_whole_text, keywords
                )  # create_keyword_network( sized_down_whole_text, topic_extraction(whole_str ,"gpt-4-turbo" ).split("," ))
...
uploaded_files = st.file_uploader(
    "Upload multiple PDF reports here:",
    type=["pdf"],
...
if uploaded_files and not st.session_state.get("multiple_pdfs_processed", False):
    ...
    if st.button("Proceed with text processing"):
        ...
        while attempts < max_attempts and not achieved:
            try:
                if spesefic_topic == "":
                    sized_down_whole_text = truncate_to_token_limit(
                        whole_str
