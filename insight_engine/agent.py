...
class Agent:
    ...
    def answer(self, state: AgentState) -> dict:
        ...
        usertype: UserType = state.get("user_type", UserType.BD)
        audience: AudienceType = state.get("audience", AudienceType.CEO)
    ...
    async def run(
        self, inp: AgentState, config: RunnableConfig | None = None
    ) -> AsyncGenerator[StandardStreamEvent | CustomStreamEvent, Any]:
        """Runs the agent asynchronously, yielding events from the state graph.
        ...
        async for event in self.graph.astream_events(
            input=inp, config=config, version="v2"
        ):
            yield event
